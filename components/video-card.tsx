...
export const VideoCard = ({ src, desc, handleSetItem }: VideoCardProps) => {
	...
	return (
		...
		<li className="relative md:h-full md:w-auto">
			<Image
				src={generateThumbnail(src)!}
				alt={desc}
				width={1920}
				height={1080}
				className={`m-auto h-auto w-auto cursor-pointer rounded-lg object-contain max-md:h-full`}
				onClick={() => handleSetActive()}
			/>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 640 640"
				className="md: absolute right-0 bottom-0 h-fit w-1/4 fill-white"
			>
